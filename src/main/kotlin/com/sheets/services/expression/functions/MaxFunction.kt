...
@Component
class MaxFunction(private val cellService: CellService) : ExpressionFunction {
    
    private val logger = LoggerFactory.getLogger(MaxFunction::class.java)
    ...
    override fun evaluate(args: List<String>): String {
        ...
        val argsToProcess = if (args.last().startsWith("sheetId=")) {
            ...
            logger.info("MAX: Using sheetId: {}", sheetId)
            args.dropLast(1)
        } else {
            args
        }
            ...
            try {
                ...
                if (a1RangeMatcher.matches()) {
                    ...
                    if (startA1Matcher.matches() && endA1Matcher.matches()) {
                        ...
                        val rangeMax = findMaxInRangeA1(startRow, startColLetter, endRow, endColLetter, sheetId)
                        if (rangeMax != null && (max == null || rangeMax > max)) {
                            max = rangeMax
                        }
                        continue
                    }
                ...
                if (a1CellMatcher.matches()) {
                    ...
                    logger.info("MAX: Looking up cell with ID: {}", cellId)
                    val cell = cellService.getCell(cellId)
                    if (cell != null) {
                        logger.info("MAX: Found cell: {}, value: {}", cellId, cell.evaluatedValue)
                        val value = cell.evaluatedValue.toDoubleOrNull()
                        if (value != null) {
                            if (max == null || value > max) {
                                max = value
                                logger.info("MAX: Updated max to {}", max)
                            }
                        } else {
                            logger.warn("MAX: Could not convert value '{}' to double", cell.evaluatedValue)
                        }
                    } else {
                        logger.warn("MAX: Cell not found: {}", cellId)
                    }
                    continue
                }
                ...
                if (max == null || numValue > max) {
                    max = numValue
                    logger.info("MAX: Updated max to {} from direct number", max)
                }
            } catch (e: NumberFormatException) {
                logger.error("MAX: Invalid number format: {}", arg, e)
                throw FunctionEvaluationException("MAX", "Invalid number format: $arg")
            } catch (e: Exception) {
                logger.error("MAX: Error processing argument: {}", arg, e)
                throw FunctionEvaluationException("MAX", "Error processing argument: $arg - ${e.message}")
            }
        }
        
        logger.info("MAX: Final result: {}", max)
        return max?.toString() ?: "0"
    }
    
    private fun findMaxInRangeA1(startRow: Int, startColLetter: String, endRow: Int, endColLetter: String, sheetId: Long): Double? {
        var max: Double? = null
