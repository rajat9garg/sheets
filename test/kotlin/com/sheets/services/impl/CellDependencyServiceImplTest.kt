...
class CellDependencyServiceImplTest {
    ...
    fun `test createDependency creates and saves dependency`() {
        ...
        // When
        val result = cellDependencyService.createDependency(dependency)
    ...
    fun `test deleteDependenciesBySourceCellId deletes dependencies`() {
        ...
        // When
        cellDependencyService.deleteBySourceCellId(sourceCellId)
        
        // Then
        verify(exactly = 1) { cellDependencyRepository.deleteBySourceCellId(sourceCellId) }
        verify(exactly = 1) { cellDependencyRedisRepository.deleteBySourceCellId(sourceCellId) }
    }
    ...
    fun `test updateDependentCells updates dependent cells`() {
        ...
        every { cellDependencyRedisRepository.getDependenciesByTargetCellId(cellId) } returns dependencies
        ...
        // Then
        verify(exactly = 1) { cellDependencyRedisRepository.getDependenciesByTargetCellId(cellId) }
        ...
        verify(atLeast = 1) { cellLockService.acquireLock(any(), any()) }
        verify(atLeast = 1) { cellLockService.releaseLock(any(), any()) }
    ...
    fun `test updateDependentCells handles circular dependencies`() {
        ...
        assertEquals("#ERROR!", cellSlot.captured.evaluatedValue)
    }
    ...
    fun `test updateDependentCells recursively updates dependent cells`() {
        ...
        every { cellDependencyRedisRepository.getDependenciesByTargetCellId(cellId) } returns dependencies1
        every { cellDependencyRedisRepository.getDependenciesByTargetCellId("1:2:1") } returns dependencies2
        every { cellRedisRepository.getCell("1:2:1") } returns dependentCell1
        every { cellRedisRepository.getCell("1:3:1") } returns dependentCell2
        ...
        // Then
        verify(exactly = 1) { cellDependencyRedisRepository.getDependenciesByTargetCellId(cellId) }
        verify(exactly = 1) { cellDependencyRedisRepository.getDependenciesByTargetCellId("1:2:1") }
        verify(exactly = 1) { cellRedisRepository.getCell("1:2:1") }
        verify(exactly = 1) { cellRedisRepository.getCell("1:3:1") }
        verify(exactly = 1) { expressionParser.parse("=A1+10") }
